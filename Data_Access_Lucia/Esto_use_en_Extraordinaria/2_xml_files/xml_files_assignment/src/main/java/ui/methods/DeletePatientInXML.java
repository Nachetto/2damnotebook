...
public class DeletePatientInXML {

    private final PatientsService service;

    @Inject
    public DeletePatientInXML(PatientsService service) {
    ...
    public void deletePatient() {
        ...
        Either<AppError,Integer> either = service.deleteXML(patientId, confirmed);
        ...
        if (either.isRight()) {
            ...
            if(either.getLeft().getMessage().equals(Constants.PATIENT_HAS_MEDICATION_ASSOCIATED_TO_MEDICAL_RECORDS_ERROR)){
                ...
                if (input.equals("Y")) {
                    confirmed = true;
                    either = service.deleteXML(patientId, confirmed);
                    if (either.isRight()) {
                        System.out.println("Patient deleted successfully!");
                    } else {
                        System.out.println(either.getLeft().getMessage());
