package common;
public class Constants {
    public static final String BIENVENIDA = "WELLCOME BACK, ";
    public static final String USERRNOTADDED = "Error on making the user";
    public static final String USERADDED = "User added";
    public static final String CUSTOMERNOINFO = "the selected customer is empty";
    public static final String CUSTOMERDELETED = "Customer deleted";
    public static final String CUSTOMERNOTDELETED = "The customer has not been deleted";
    public static final String CUSTOMERNOTADDED = "The customer has not been added";
    public static final String ORDERADDED = "The order has been added successfully";
    public static final String ORDERITEMADDED = "The order item has been added successfully";
    public static final String ORDERITEMREMOVED = "The order item has been successfully removed";
    public static final String ORDERDELETED = "The order has been deleted successfully";
    public static final String ORDERUPDATED = "The order has been updated successfully";
    public static final String CONFIRMUSERDELETION = "Are you sure that you want to delete this user? Its orders and order items will be deleted as well.";
    public static final String CUSTOMERDOESNTEXIST = "The id of the customer does not appear on our database.";
    public static final String ORDERDBERROR = "There was an error while reading Orders Database: ";
    public static final String CUSTOMERDBERROR = "Error al leer los datos de los clientes: ";
    public static final String ERRDB = "There was an error while trying to connect to the database supplier\n";
    public static final String ORDER_ITEM_DB_ERROR = "There was an error while reading Order Items Database: ";
    public static final String ID_NOT_FOUND = "The requested id has not found any item";
    public static final String ERROR_OBTAINING_ORDER_ITEM = "There was an error while trying to obtain the order item: ";
    public static final String IDNOTFOUND = "The requested id has not found any item";
    public static final String ERROROBTAININGORDER = "There was an error while trying to obtain the order: ";
    public static final String MENUITEMSXMLREADERROR = "There was an error while reading the menu items from XML file";
    public static final String ERRORSAVE = "There was an error while trying to save the menu item: ";
    public static final String ERRORMODIFY = "There was an error while trying to modify the menu item: ";
    public static final String ERRORSAVING = "There was an error while trying to save the order item: ";
    public static final String ERRORDELETE = "There was an error while trying to delete the order item: ";
    public static final String ORDERNOTDELETED = "The order has not been deleted";
    public static final String ORDERITEMDBERROR = "There was an error while reading Order Items Database: ";
    public static final String CUSTOMERNOTUPDATED = "The customer has not been updated";
    public static final String CUSTOMERUPDATED = "The customer has been updated";
    public static final String MENUITEMDBERROR = "There was an error while reading Menu Items Database: ";
    public static final String ERROROBTAININGMENUITEM = "There was an error while trying to obtain the menu item: ";
    public static final String ERROR_FILE_NOT_FOUND = "The file has not been found";
}
