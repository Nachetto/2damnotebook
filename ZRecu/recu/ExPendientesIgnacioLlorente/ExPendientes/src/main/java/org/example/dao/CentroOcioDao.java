...
public class CentroOcioDao {
    ...
    public CentroOcioDao() {
        ...
        try {
            ...
            centros.add(new ParqueAtracciones(3, "parque3", (int) (Math.random() * 10) + 16, "provincia3", fechaAleatoria(), "correo3@gmail.com", (int) (Math.random() * 5) + 1, 0));
            centros.add(new ParqueAtracciones(4, "parque4", (int) (Math.random() * 10) + 16, "provincia4", fechaAleatoria(), "correo4@gmail.com", (int) (Math.random() * 5) + 1, 0));
            //Zoologicos
            centros.add(new Zoologico(5, "parque5", (int) (Math.random() * 10) + 11, "provincia5", fechaAleatoria(), "correo5@gmail.com", (int) (Math.random() * 5) + 1, servicios1));
            centros.add(new Zoologico(6, "parque6", (int) (Math.random() * 10) + 11, "provincia6", fechaAleatoria(), "correo6@gmail.com", (int) (Math.random() * 5) + 1, servicios2));
            centros.add(new Zoologico(7, "parque7", (int) (Math.random() * 10) + 11, "provincia7", fechaAleatoria(), "correo7@gmail.com", (int) (Math.random() * 5) + 1, servicios3));
            centros.add(new Zoologico(8, "parque8", (int) (Math.random() * 10) + 11, "provincia8", fechaAleatoria(), "correo8@gmail.com", (int) (Math.random() * 5) + 1, servicios4));
        } catch (Exception e) {
    ...
    public List<CentroOcio> listadoOrdenadoProvinciaFecha() {
        ...
        copialista.sort((o1, o2) -> {
            int comparator = o1.getProvincia().compareTo(o2.getProvincia());
            if (comparator == 0)
                comparator = o1.getFechaConstruccion().compareTo(o2.getFechaConstruccion());
            return comparator;
        });

        //esto lo he visto en internet y me ha parecido interesante
        //Comparator.comparing(CentroOcio::getProvincia).thenComparing(CentroOcio::getFechaConstruccion)
    ...
    public List<ParqueAtracciones> listadoOrdenadoParqueAtracciones(int edad, boolean festivo) {
        ...
        /* si me quiero complicar mas:
        return getCentros().stream()
                .filter(centro -> centro instanceof ParqueAtracciones)
                .map(centro -> (ParqueAtracciones) centro)
                .sorted(Comparator.comparing(ParqueAtracciones::getProvincia)
                        .thenComparing(parque -> parque.precioEntrada(edad, festivo))).toList();
        */
    }
    ...
    public int removeCentrosOcio(int anyo) {
        List<CentroOcio> centrosAEliminar = new ArrayList<>();
        for (CentroOcio centroOcio : centros) {
            if (centroOcio.getFechaConstruccion().getYear() < anyo)
                centrosAEliminar.add(centroOcio);
        }
        centros.removeAll(centrosAEliminar);
        return centrosAEliminar.size();
    }
    ...
    public Map<String, List<CentroOcio>> mapa() {
        Map<String, List<CentroOcio>> mapa = new HashMap<>();
        List<CentroOcio> listaCentros = new ArrayList<>();
        for (CentroOcio centro : centros) {
            if (mapa.containsKey(centro.getProvincia())) {
                listaCentros = mapa.get(centro.getProvincia());
                listaCentros.add(centro);
                mapa.put(centro.getProvincia(), listaCentros);
            } else {
